...
class DeltaLog private(
  ...
  def startTransaction(
    ...
    TransactionExecutionObserver.threadObserver.get().startingTransaction {
      logWarning(
        s"[Omni-Proof][DeltaWrite] DeltaLog.startTransaction resolved to " +
          s"OmniOptimisticTransaction; tablePath=$dataPath, " +
          s"catalogTable=${catalogTableOpt.map(_.identifier.table).getOrElse("<path-based>")}, " +
          s"snapshotVersion=${snapshotOpt.map(_.version).getOrElse("latest")}")
      new OmniOptimisticTransaction(this, catalogTableOpt, snapshotOpt)
    }
  }
