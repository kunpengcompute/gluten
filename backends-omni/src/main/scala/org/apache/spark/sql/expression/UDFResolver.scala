...
object UDFResolver extends Logging {
  ...
  def resolveUdfConf(sparkConf: SparkConf, isDriver: Boolean): Unit = {
    val udfLibPaths = if (isDriver) {
      sparkConf
        .getOption(OmniBackendSettings.GLUTEN_OMNI_DRIVER_UDF_LIB_PATHS)
        .orElse(sparkConf.getOption(OmniBackendSettings.GLUTEN_OMNI_UDF_LIB_PATHS))
    } else {
      sparkConf.getOption(OmniBackendSettings.GLUTEN_OMNI_UDF_LIB_PATHS)
    }
    ...
    udfLibPaths match {
      case Some(paths) =>
        ...
        sparkConf.set(
          OmniBackendSettings.GLUTEN_OMNI_INTERNAL_UDF_LIB_PATHS,
  ...
  private def checkAllowTypeConversion: Boolean = {
    SQLConf.get
      .getConfString(OmniBackendSettings.GLUTEN_OMNI_UDF_ALLOW_TYPE_CONVERSION, "false")
      .toBoolean
  }
