...
case class OmniWindowGroupLimitExecTransformer(
  ...
  def getWindowGroupLimitRel(
    ...
    if (!validation) {
      ...
      val extensionNode = ExtensionBuilder.makeAdvancedExtension(
        genWindowGroupLimitParameters(rankLikeFunction),
  ...
  def genWindowGroupLimitParameters(rankLikeFunction: Expression): Any = {
    val paramsStr = new StringBuffer("WindowGroupLimitParameters:")
    val funcName = rankLikeFunction match {
      case _: Rank => "rank"
      case _: RowNumber => "row_number"
      case other => other.prettyName
    }
    if (funcName == "rank") {
      paramsStr.append("isRank=").append(1).append("\n")
    } else if (funcName == "row_number") {
      paramsStr.append("isRowNumber=").append(1).append("\n")
    } else {
      paramsStr.append("isOther=").append(1).append("\n")
    }

    val message = StringValue
      .newBuilder()
      .setValue(paramsStr.toString)
      .build()
  ...
  override protected def doTransform(context: SubstraitContext): TransformContext = {
    ...
    val currRel =
      getWindowGroupLimitRel(context, child.output, operatorId, childCtx.root, validation = true)
