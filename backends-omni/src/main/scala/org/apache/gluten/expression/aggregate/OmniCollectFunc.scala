...
abstract class OmniCollectFunc(child: Expression)
...
case class OmniCollectSet(child: Expression) extends OmniCollectFunc(child) {

  override lazy val evaluateExpression: Expression =
    ArrayDistinct(buffer)

  override def prettyName: String = "omni_collect_set"

  override protected def withNewChildInternal(newChild: Expression): Expression =
    copy(child = newChild)
}

case class OmniCollectList(child: Expression) extends OmniCollectFunc(child) {

  override val evaluateExpression: Expression = buffer

  override def prettyName: String = "omni_collect_list"
