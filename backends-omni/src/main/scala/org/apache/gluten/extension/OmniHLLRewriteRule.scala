...
case class OmniHLLRewriteRule(spark: SparkSession) extends Rule[LogicalPlan] {
  ...
  private def isSupportedDataType(dataType: DataType): Boolean = {
    // HLL in Omni only supports below data types. We should not offload HLL to Omni if
    // child's data type is not supported. This prevents the case only partial agg is fallbacked.
    // As Spark and Omni have different HLL binary formats, HLL binary generated by Spark can't
    // be parsed by Omni, it would cause the error: 'Unexpected type of HLL'.
    dataType match {
