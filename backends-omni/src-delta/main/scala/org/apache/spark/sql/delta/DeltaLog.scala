...
/**
 * Gluten-Omni copy of Delta Lake's DeltaLog with targeted metadata-read adjustments.
 *
 * The class keeps Delta's transaction and snapshot semantics, while allowing `_delta_log` JSON
 * commit files to be read through Delta's LogStore/action parsing path instead of Spark JSON scan
 * plans that would otherwise enter Omni's native scan pipeline.
 *
...
class DeltaLog private(
  ...
  def loadIndex(
    ...
    if (index.format == DeltaLogFileIndex.COMMIT_FILE_FORMAT) {
      loadCommitIndex(index, schema)
    } else {
      Dataset.ofRows(spark, indexToRelation(index, schema))
    }
  }

  private def loadCommitIndex(
      index: DeltaLogFileIndex,
    ...
    val actions: Seq[(SingleAction, Long)] = index.inputFiles.sorted.iterator.flatMap {
      pathString =>
        val path = new Path(pathString)
        val version = deltaVersion(path)
        store.read(path, hadoopConf).map { json =>
          val action: SingleAction = Action.fromJson(json).wrap
          (action, version)
        }
    }.toSeq

    if (actions.isEmpty) {
      spark.createDataFrame(spark.sparkContext.emptyRDD[Row], Action.logSchema)
        .withColumn(versionCol, lit(null).cast("long"))
        .select((schema.fieldNames.map(col) :+ col(versionCol)): _*)
    } else {
      spark.createDataset[(SingleAction, Long)](actions)
        .toDF("action", versionCol)
        .select(
          (schema.fieldNames.map(name => col(s"action.$name").as(name)) :+ col(versionCol)): _*)
    }
  }
  ...
  private[delta] def getCanonicalPathUdf(runsOnExecutors: Boolean = true): UserDefinedFunction = {
    DeltaUDF
      .stringFromString(getCanonicalPathFunction(runsOnExecutors))
      .withName("delta_canonical_path")
  }
}

/**
 * Companion object for the Gluten-Omni DeltaLog copy.
 *
 * It preserves Delta's factory/cache behavior so callers keep using the standard
 * `org.apache.spark.sql.delta.DeltaLog` API when the Delta profile is enabled.
 */
object DeltaLog extends DeltaLogging {
  ...
  private class DeltaHadoopFsRelation(
    ...
    )(spark: SparkSession, deltaLog: DeltaLog, catalogTableOpt: Option[CatalogTable])
